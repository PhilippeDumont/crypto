...
1) Take the message
2) Converte all the character in ASCII
3) Calcul all ASCII code with (ASCII)^C mod(N)
4) Mesage is encrypted
"""
...
class Interface(object):
    ...
    def __init__(self, root, rsa):
        ...
        self.labelP.grid(column=0, row=0, padx=5, sticky=(N))
        self.labelQ.grid(column=0, row=1, sticky=(N))
        self.labelC.grid(column=0, row=3, sticky=(N))
        self.labelN.grid(column=0, row=4, sticky=(N))
        self.labelU.grid(column=0, row=5, sticky=(N))
        ...
        self.inputN.grid(column=1, row=4)
